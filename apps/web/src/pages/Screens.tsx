...
import type { Alert, EnforcementCase, HistoricalReading, SensorReading } from '../lib/types';
...
export function AlertsPage() {
  ...
  const { activeCityId } = useCity();
  const [page, setPage] = useState(1);
  const alerts = useQuery({ queryKey: ['alerts', activeCityId, page], queryFn: () => api.alertsPage({ cityId: activeCityId, page, limit: 8 }), refetchInterval: false });
  ...
  const filtered = alerts.data?.data.filter((item) => filter === 'all' || item.severity === filter) ?? [];
  return <div className="page alerts-page">
    ...
    <div className="alert-layout"><Panel className="alert-feed" title="Live alert feed" subtitle={`${filtered.length} signals in current view`}>{alerts.isLoading ? <LoadingState /> : alerts.error ? <ErrorState message={alerts.error.message} /> : !filtered.length ? <EmptyState title="No matching alerts" description="The selected severity queue is clear." /> : <><div className="alert-list">{filtered.map((item) => <button key={item.id} className={`alert-card severity-${item.severity} ${selected?.id === item.id ? 'selected' : ''}`} onClick={() => setSelected(item)}><div><StatusChip value={item.severity} /><StatusChip value={item.status} />{!item.readAt && <StatusChip value="unread" />}<span>{relativeTime(item.createdAt)}</span></div><strong>{item.title}</strong><p>{item.description}</p><small>{item.source} - {item.ward}</small></button>)}</div>{alerts.data && <PaginationControls {...alerts.data} onPage={setPage} />}</>}</Panel><div className="alert-intelligence"><Panel title="Alert correlation AI" subtitle="Related signals grouped by time and location">{correlations.isLoading ? <LoadingState /> : <div className="correlation-list">{correlations.data?.clusters.map((cluster) => <article key={cluster.id}><Layers3 /><div><strong>{cluster.summary}</strong><span>{cluster.alertIds.length} source signals - {Math.round(cluster.confidence * 100)}% confidence</span></div><StatusChip value={cluster.severity} /></article>)}</div>}</Panel>{selected ? <Panel className="selected-alert" title="Selected incident" subtitle={selected.id}><div className="incident-title"><Siren /><div><StatusChip value={selected.severity} /><h3>{selected.title}</h3></div></div><p>{selected.description}</p><dl><div><dt>LOCATION</dt><dd>{selected.ward}</dd></div><div><dt>SOURCE</dt><dd>{selected.source}</dd></div><div><dt>STATUS</dt><dd>{selected.status}</dd></div></dl><div className="form-actions">{!selected.readAt && <button className="button ghost" onClick={async () => { await api.markAlertRead(selected.id); await client.invalidateQueries({ queryKey: ['alerts'] }); setSelected(undefined); }}><Check />Mark read</button>}{selected.status === 'open' && <button className="button secondary" onClick={() => mutation.mutate({ item: selected, status: 'acknowledged' })}>Acknowledge</button>}{selected.status !== 'resolved' && <button className="button primary" onClick={() => mutation.mutate({ item: selected, status: 'resolved' })}><Check />Resolve incident</button>}</div></Panel> : <IntelligenceBrief><p>Select an alert to review evidence, ownership, and response controls.</p></IntelligenceBrief>}</div></div>
  </div>;
}
...
export function HistoricalPage() {
  ...
  const exportRows = () => downloadCsv(`airiq-${activeCityId}-historical.csv`, (query.data?.data ?? []) as unknown as Array<Record<string, unknown>>);
  return <div className="page">
    <PageHeader eyebrow="HISTORICAL DATA" title="Historical Data" description="Review ETL readings by date range and export the current filtered page." actions={<button className="button primary" onClick={exportRows} disabled={!query.data?.data.length}><FileDown />Export CSV</button>} />
