...
export function ConfidenceChart({ data }: { data: ForecastPoint[] }) {
  ...
  return <div className="chart chart-short"><ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={180}><BarChart data={chartData}><CartesianGrid stroke="#20303c" strokeDasharray="2 4" vertical={false} /><XAxis dataKey="horizon" stroke="#6e8492" axisLine={false} tickLine={false} fontSize={10} /><YAxis domain={[0, 100]} hide /><Tooltip contentStyle={tooltipStyle} /><Bar dataKey="confidence" radius={[3, 3, 0, 0]} fill="#47e6a5" /></BarChart></ResponsiveContainer></div>;
}
...
export function CityTrendChart({ cities }: { cities: City[] }) {
  ...
  return <div className="chart"><ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={220}><LineChart data={chartData} margin={{ top: 10, right: 10, bottom: 0, left: -18 }}><CartesianGrid stroke="#20303c" strokeDasharray="2 4" vertical={false} /><XAxis dataKey="day" stroke="#6e8492" axisLine={false} tickLine={false} fontSize={10} /><YAxis stroke="#6e8492" axisLine={false} tickLine={false} fontSize={10} /><Tooltip contentStyle={tooltipStyle} />{cities.slice(0, 4).map((city, index) => <Line key={city.id} dataKey={city.name} type="monotone" stroke={colors[index]} dot={false} strokeWidth={2} />)}</LineChart></ResponsiveContainer></div>;
}
...
export function ContributionComparison({ cities }: { cities: City[] }) {
  ...
  return <div className="chart"><ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={220}><BarChart data={data} layout="vertical" margin={{ left: 22, right: 10 }}><CartesianGrid stroke="#20303c" strokeDasharray="2 4" horizontal={false} /><XAxis type="number" hide /><YAxis dataKey="name" type="category" stroke="#8ca0ad" axisLine={false} tickLine={false} fontSize={10} /><Tooltip contentStyle={tooltipStyle} /><Bar dataKey="traffic" stackId="a" fill="#ff6d6d" /><Bar dataKey="dust" stackId="a" fill="#ffb84d" /><Bar dataKey="industry" stackId="a" fill="#54d8ff" /><Bar dataKey="other" stackId="a" fill="#778b98" /></BarChart></ResponsiveContainer></div>;
}

export function Sparkline({ values }: { values: number[] }) {
  const data = values.map((value, index) => ({ index, value }));
  return <div className="sparkline" aria-label="24 hour AQI sparkline"><ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={30}><LineChart data={data}><Line type="monotone" dataKey="value" stroke="#54d8ff" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></div>;
}
